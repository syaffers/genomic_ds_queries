...
do
    ...
    { time gemini query --header -q "select * from variants where rs_ids='$rs'" /media/shared/tools/gemini/153_gemini_pass_normalized.db > /dev/null; } 2>&1 > /dev/null | cut -d '	' -f 2 | head -2 | tail -1 | ../utils/m2s.py >> $filename
