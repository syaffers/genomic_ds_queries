filename="results_snpsift_wchr.txt"
...
do
    ...
    { time java -jar /media/shared/tools/snpEff/SnpSift.jar filter "( ID = '$rs' & CHROM='$chr' )" /media/shared/paper-queries/arab1k.vcf > /dev/null; } 2>&1 > /dev/null | tail -3 | cut -d '	' -f 2 | head -2 | tail -1 | ../utils/m2s.py >> $filename
