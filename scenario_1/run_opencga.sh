...
do
    ...
    { time docker exec -it opencga-rest2 ./opencga.sh variant query -s arab1k --id $rs --of JSON > /dev/null; } 2>&1 > /dev/null | tail -3 | cut -d '	' -f 2 | head -1 | ../utils/m2s.py >> $filename
